<template>
    <div>
        <div v-if="ok">
            <h4 class="mt-3 mb-5">Вы действительно хотите удалить {{ skill.Name }}?</h4>
            <div class="text-danger my-3" v-for="error in v$.skill.$errors" :key="error.$uid">{{ error.$message }}</div>
    ...
    export default {
        ...
        methods: {
            validate (status) {
                var errors = [];
                if (status == 401)
                    errors = { skill: [validation.UNAUTHORIZED] };
                else
                    errors = { skill: [validation.MUST_BE_EMPTY] };
